...
typedef enum detect_eq_states { DEQInit, QuakeDet, noQuakeDet  } detect_eq_states;
typedef enum detect_max_amp_states { DMAInit,  maxAmp  } detect_max_amp_states;
typedef enum detect_zc_states { DZCInit,                                    } detect_zc_states;
typedef enum transmit_states {TInit,  TOut } transmit_states;

/* shared variables --- fill in as needed */

const unsigned char period = 100;
unsigned char ping = 0x00;
//unsigned detected = 0x00;
